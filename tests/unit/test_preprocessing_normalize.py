...
def test_unit_scheme_maps_255_to_1() -> None:
    ...
    normalize_into(dst, src, scheme="unit", input_order="BGR", output_order="BGR")
...
def test_signed_scheme_maps_0_and_255_to_minus_one_and_one() -> None:
    src = _rgb_img()
    dst = np.empty(src.shape, dtype=np.float32)
    normalize_into(dst, src, scheme="signed", input_order="BGR", output_order="BGR")
    assert dst.max() == pytest.approx(1.0)
...
def test_imagenet_scheme_subtracts_mean_and_divides_std() -> None:
    ...
    normalize_into(dst, src, scheme="imagenet", input_order="BGR", output_order="RGB")
...
def test_channel_swap_flips_bgr_to_rgb() -> None:
    ...
    dst = np.empty(src.shape, dtype=np.float32)
    normalize_into(dst, src, scheme="unit", input_order="BGR", output_order="RGB")
...
def test_normalize_into_reuses_dst_buffer_identity() -> None:
    src = _rgb_img()
    dst = np.empty(src.shape, dtype=np.float32)
    result = normalize_into(dst, src, scheme="unit", input_order="BGR", output_order="BGR")
